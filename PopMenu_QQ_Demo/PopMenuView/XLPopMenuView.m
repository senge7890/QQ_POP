                       ...
                       action:(void (^)(NSInteger index))action
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.popMenuTableView.layer.anchorPoint = CGPointMake(1.0, 0);// 锚点 为右上角
...
{
    /*
        变化前的X(Y)坐标 = 变化后的X(Y)坐标 + （变化后的锚点X(Y) - 变化前的锚点X(Y)）* View的宽(高)
     变化后的坐标就是 最终需要显示的位置
     */
    CGFloat menuViewX = self.startPoint.x + 75 ;
    CGFloat menuViewY = self.startPoint.y - 0.5 * self.menuItems.count * CELLHEIGHT;
